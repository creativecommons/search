  ...
  $("#addOpenSearch").click(function () {
    if (
      ...
      window.external.AddSearchProvider(
        "http://oldsearch.creativecommons.org/ccsearch.xml"
      );
    } else {
...
function setCommDeriv() {
  if ($("#comm").attr("checked")) comm = 1;
  else comm = 0;
  if ($("#deriv").attr("checked")) deriv = 1;
  else deriv = 0;
...
function updateCommDerivCheckboxes(comOverride, derivOverride) {
  if (comm == 1) $("#comm").attr("checked", true);
  else $("#comm").attr("checked", false);
  if (deriv == 1) $("#deriv").attr("checked", true);
  else $("#deriv").attr("checked", false);
}
...
function modRights() {
  ...
  switch (engine) {
    ...
    case "europeana":
      ...
      if (comm && deriv) {
        rights =
          "+AND+RIGHTS%3A*creative*+AND+NOT+RIGHTS%3A*nc*+AND+NOT+RIGHTS%3A*nd*";
      } else if (comm && !deriv) {
  ...
  if (query.val().length > 0 /* && (query.className == "active")*/) {
    ...
    switch (engine) {
      ...
      case "jamendo":
        if (rights) {
          url =
            "http://www.jamendo.com/search?qs=fq=license_cc:(" +
            rights +
            ")&q=" +
            query.val();
        } else {
      ...
      case "owlmm":
        url =
          "http://www.owlmm.com/?query_source=CC&" +
          (rights.length > 13 ? rights : "license_type=cc") +
          "&q=" +
          query.val();
        break;
      ...
      case "wikimediacommons":
        url =
          "http://commons.wikimedia.org/w/index.php?title=Special%3ASearch&redirs=0&search=" +
          query.val() +
          "&fulltext=Search&ns0=1&ns6=1&ns14=1&title=Special%3ASearch&advanced=1&fulltext=Advanced+search";
        break;

      case "fotopedia":
        url =
          "http://www.fotopedia.com/search?q=" +
          query.val() +
          "&human_license=" +
          rights;
        break;
      ...
      case "ccmixter":
        url =
          "http://ccmixter.org/api/query?datasource=uploads&search_type=all&sort=rank&search=" +
          query.val() +
          rights;
        break;
    ...
    url =
      "https://oldsearch.creativecommons.org/bouncer.php?q=" +
      query.val() +
      "&url=" +
